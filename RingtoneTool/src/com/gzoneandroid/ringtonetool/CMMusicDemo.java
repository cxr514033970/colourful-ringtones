	...
	private class UIHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				if (msg.obj == null) {
					hideProgressBar();
					Toast.makeText(CMMusicDemo.this, R.string.result_equals_null, Toast.LENGTH_SHORT).show();
					return;
				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				dialog = ProgressDialog.show(CMMusicDemo.this, null, getString(R.string.please_wait), true, false);
					...
					public void operationResult(Result result) {
						if (null != result) {
							new AlertDialog.Builder(CMMusicDemo.this).setTitle("login")
									.setMessage(result.toString()).setPositiveButton(R.string.confirm, null)
									.show();
						}
					...
					public void callback(final String phoneNum) {
						...
						new Thread() {
							...
							public void run() {
								...
								CrbtOpenCheckRsp result= RingbackManagerInterface.crbtOpenCheck(CMMusicDemo.this, phoneNum);
					...
					public void callback(final String singerId) {
						...
						new Thread() {
							...
							public void run() {
								...
								SingerInfoRsp result= MusicQueryInterface.getSingerInfo(CMMusicDemo.this, singerId);
					...
					public void callback(final String musicId) {
						...
						new Thread() {
							...
							public void run() {
								...
								AlbumListRsp result= MusicQueryInterface.getAlbumsByMusicId(CMMusicDemo.this,
										musicId, 1, 5);
...
//						});
		...
		musicQuery.setOnClickListener(new OnClickListener() {
			String[] strs = new String[] { "获取榜单信息", "获取榜单音乐信息", "获取专辑信息", "获取专辑音乐信息", "获取歌手音乐信息",
			...
			public void onClick(View v) {
				...
				new AlertDialog.Builder(CMMusicDemo.this).setTitle("歌曲查询类")
						.setItems(getResources().getStringArray(R.array.songslist_type), new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
									...
									new Thread() {
										...
										public void run() {
											MusicListRsp m = MusicQueryInterface
													.getMusicsByChartId(CMMusicDemo.this, "101", 1,
															5);
								...
								case 2:
									...
									new Thread() {
										...
										public void run() {
											AlbumListRsp a = MusicQueryInterface
													.getAlbumsBySingerId(CMMusicDemo.this, "235",
															1, 5);
									...
									break;
								case 3:
									showParameterDialog(getString(R.string.album_id), new ParameterCallback() {
										...
										public void callback(final String albumId) {
											...
											new Thread() {
												...
												public void run() {
													MusicListRsp m = MusicQueryInterface
															.getMusicsByAlbumId(CMMusicDemo.this, albumId, 1, 5);
								...
								case 4:
									...
									new Thread() {
										...
										public void run() {
											MusicListRsp m = MusicQueryInterface
													.getMusicsBySingerId(CMMusicDemo.this, "235",
															1, 5);
								...
								case 6:
									...
									new Thread() {
										@Override
										public void run() {
											MusicListRsp t = MusicQueryInterface.getMusicsByTagId(
													CMMusicDemo.this, "100", 1, 5);
									...
									new Thread() {
										...
										public void run() {
											...
											t = MusicQueryInterface.getMusicsByKey(
													CMMusicDemo.this, URLEncoder.encode(getString(R.string.youdiantian)), 1,
													5);
											mUIHandler.obtainMessage(0, t).sendToTarget();
										}
									}.start();
						...
						RingbackManagerInterface.buyRingbackByNet(CMMusicDemo.this, musicId, false,
								new CMMusicCallback<Result>() {
									...
									public void operationResult(Result ret) {
										if (null != ret) {
											new AlertDialog.Builder(CMMusicDemo.this)
													.setTitle("buyRingback").setMessage(ret.toString())
													.setPositiveButton(R.string.confirm, null).show();
										}
									}
								});
		...
		vRing.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				showParameterDialog("musicId", new ParameterCallback() {
					...
					public void callback(final String musicId) {
						Log.i("TAG", "musicId = "+musicId);
						VibrateRingManagerInterface.queryVibrateRingDownloadUrlByNet(CMMusicDemo.this,
		...
		openFullSong.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
//				FullSongManagerInterface.openSongMonthByNet(CMMusicDemo.this, false,
//						new CMMusicCallback<Result>() {
//							@Override
//							public void operationResult(Result ret) {
//								if (null != ret) {
				...
				showParameterDialog("musicId", new ParameterCallback() {
					...
					public void callback(final String musicId) {
						...
						new Thread() {
							...
							public void run() {
								super.run();
								StreamRsp s = OnlineListenerMusicInterface.getStream(CMMusicDemo.this,
										musicId);
	...
	public void onClick(View arg0) {
			...
			RingbackManagerInterface.deletePersonRing(this, "60078701600",
					new CMMusicCallback<Result>() {
						...
						public void operationResult(Result result) {
							if (null != result) {
								new AlertDialog.Builder(CMMusicDemo.this)
										.setTitle("deletePersonRing").setMessage(result.toString())
										.setPositiveButton(R.string.confirm, null).show();
							}
		...
		case R.id.fullsong:
			...
			showParameterDialog("musicId", new ParameterCallback() {
				...
				public void callback(final String musicId) {
					Log.i("TAG", "musicId = "+musicId);
					FullSongManagerInterface.getFullSongDownloadUrlByNet(CMMusicDemo.this, musicId, false,
